    ...
    def _sync_external_memory_for_turn(
        ...
        *,
        ...
        try:
            ...
            observed_messages = getattr(
                self, "_memory_observed_context_messages", None
            )
            ...
            if messages is not None:
                if observed_messages:
                    ...
                    if observed_for_sync:
                        # OpenViking slices the current turn by walking
                        # backward from the addressed user row. Keep observed
                        # rows immediately before that row so their individual
                        # memory sources and peer IDs remain attributable.
                        insert_at = 0
                        for idx in range(len(messages_for_sync) - 1, -1, -1):
                            candidate = messages_for_sync[idx]
                            if (
                                isinstance(candidate, dict)
                                and candidate.get("role") == "user"
                                and not candidate.get("observed")
                            ):
                                insert_at = idx
                                break
            ...
            memory_context = getattr(self, "_memory_turn_context", None)
            ...
            if memory_context is not None:
                prefetch_kwargs["context"] = memory_context
